...
export default function Sidebar({ setAdminLoggedIn }){
    return (
        <div className='bg-slate-800 w-60 p-3 flex flex-col text-white'>
            ...
            <div className='flex flex-col gap-0.5 pt-2 border-t border-neutral-700'>
                {Sidebar_Links_Bottom.map((item) =>  (
                    <SidebarLink key={item.key} item={item} setAdminLoggedIn={ setAdminLoggedIn }/>
                ))}
            </div>
        </div>
    )
...
function SidebarLink({item, setAdminLoggedIn }){
